...
public class Bedroom2 extends HomeRelation {
	
	private static final int MODE_ON = 1;	// light on, humidifier on	
	private static final int MODE_OFF = 2;	// light off, humidifier off
	...
	public Bedroom2(int prelocation, int location1, int location2, int location3, double temperature, double humidity,
			double co2) {
	...
	public void start(GWProcess process, HomeApplianceManager haMgr) {
		switch(predict) {
		case MODE_ON:
			if(!haMgr.equalOnOff(UtasDeviceType.DIMMING, UtasDeviceNo.NO_5, UtasDimmingFunction.DIM_100)) {				
				process.controlDevice(PacketSHCollection.dimming5(UtasDimmingFunction.DIM_100), UtasPacketTimer.INTERVAL_CTR);
			}
			
			if(!haMgr.equalOnOff(UtasDeviceType.HUMIDIFIER, UtasDeviceNo.NO_1, UtasHumidifierFunction.ON)) {
				process.controlDevice(PacketSHCollection.humidifier1(UtasHumidifierFunction.ON), UtasPacketTimer.INTERVAL_CTR);
			}
