...
public class Entry extends HomeRelation {
	private static final int MODE_ON = 1;		// air condition on, dehumidifier on, air cleaner on, robot vacuum off
	private static final int MODE_OFF = 2;		// air condition off, dehumidifier off, air cleaner off
	private static final int MODE_PER_ON = 3;	// light on, dehumidifier on,  air cleaner on
	private static final int MODE_PER_OFF = 4;	// light off, dehumidifier off,  air cleaner off
	...
	public Entry(int prelocation, int location1, int location2, int location3, double temperature, double humidity,
			double co2) {
	...
	public void start(GWProcess process, HomeApplianceManager haMgr) {
		...
		case MODE_ON:
			...
			if(!haMgr.equalOnOff(UtasDeviceType.ROBOTVACUUM, UtasDeviceNo.NO_1, UtasRobotvacuumFunction.CHARGING)) {
				process.controlDevice(PacketSHCollection.robotvacuum1(UtasRobotvacuumFunction.CHARGING), UtasPacketTimer.INTERVAL_CTR);
			}
			break;
		case MODE_OFF:
